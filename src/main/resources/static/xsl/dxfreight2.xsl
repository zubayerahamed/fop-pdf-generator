		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="DXFREIGHTLABEL" xsl:use-attribute-sets="default.font.size">
				<fo:flow flow-name="xsl-region-body">
					<fo:block-container reference-orientation="270" height="10%" width="45%" right="0" position="absolute">
								...
								<fo:table-body>
									<fo:table-row >
										<fo:table-cell number-rows-spanned="3" xsl:use-attribute-sets="border.right">
											...
											<fo:block font-weight="bold" margin-left="2px">
												CARTON (<xsl:value-of select="contentDescription"></xsl:value-of>)
											</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="border.right">
											<fo:block>
												<fo:block-container height="15px">
													<fo:block></fo:block>
												</fo:block-container>
											</fo:block>
										</fo:table-cell>
										...
										<fo:table-cell number-rows-spanned="3" xsl:use-attribute-sets="">
											...
											<fo:block font-family="sans-serif" font-weight="bold" font-size="60px" xsl:use-attribute-sets="text.center">
									...
									<fo:table-row>
										<fo:table-cell number-columns-spanned="2" xsl:use-attribute-sets="border.right border.top border.bottom">
											<fo:block>
												...
												<xsl:choose>
													<xsl:when test="$sd != 'Overnight'">
														<fo:block-container height="40px" background-color="#ffffff">
															<fo:block></fo:block>
														</fo:block-container>
													</xsl:when>
													<xsl:otherwise>
														<fo:block-container height="40px" background-color="#000000">
															<fo:block></fo:block>
														</fo:block-container>
													</xsl:otherwise>
												</xsl:choose>
										...
										<fo:table-cell number-columns-spanned="2" xsl:use-attribute-sets="border.right">
											<fo:block>
												<fo:block-container height="30px">
													<fo:block font-family="sans-serif" font-size="30px" font-weight="bold" text-align="center">B</fo:block>
												</fo:block-container>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
					...
					<fo:block-container xsl:use-attribute-sets="border.full text.center" height="25px" left="100px" width="150px" position="absolute">
						<fo:block>
							<!-- <xsl:variable name="barcode" select="trackingNumber"></xsl:variable>
							<fo:instream-foreign-object>
								<barcode:barcode xmlns:barcode="http://barcode4j.krysalis.org/ns" message="1234">
									<barcode:code39>
										<barcode:height>10px</barcode:height>
										<barcode:module-width>100px</barcode:module-width>
					...
					<fo:block-container height="14px" width="87" left="138" top="34" reference-orientation="270" position="absolute">
						<fo:block text-align="center" font-size="13px">
							Barcode
						</fo:block>
					</fo:block-container>
				</fo:flow>
