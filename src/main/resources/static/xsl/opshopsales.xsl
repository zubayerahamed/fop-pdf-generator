...
<xsl:stylesheet 
	...
	<xsl:template match="outletSalesSummary">
				...
				<fo:static-content flow-name="header-first">
					...
					<fo:block-container width="100%" margin-top="15px">
						...
						<fo:block text-align="center" font-size="9px" margin-top="5px">
							<xsl:value-of select="outletAddress"/>
						</fo:block>
							...
							<fo:table table-layout="fixed" width="100%" border-collapse="collapse">
								...
								<fo:table-body xsl:use-attribute-sets="table.font.size">
									...
									<fo:table-row>
										...
										<fo:table-cell xsl:use-attribute-sets="dealer.table.td">
											<fo:block>
												<xsl:value-of select="fromDate"/> to <xsl:value-of select="toDate"/> &#160; <xsl:value-of select="noOfSelectedDays"/>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
								</fo:table-body>
								...
								<fo:table-header xsl:use-attribute-sets="table.font.size">
									<fo:table-row>
										...
										<fo:table-cell xsl:use-attribute-sets="client.table.th">
											<fo:block>Shop</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.th" text-align="right">
											<fo:block>Net Total</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.th" text-align="right">
											<fo:block>CC</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.th" text-align="right">
											<fo:block>AVB</fo:block>
										</fo:table-cell>
									</fo:table-row>
								</fo:table-header>
								<fo:table-footer>
									<fo:table-row>
										<fo:table-cell number-columns-spanned="2" xsl:use-attribute-sets="client.table.tf" text-align="right">
											<fo:block>Grand Total</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.tf" text-align="right">
											<fo:block>
												<xsl:value-of select="grandNetTotal"/>
											</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.tf" text-align="right">
											<fo:block>
												<xsl:value-of select="grandCC"/>
											</fo:block>
										</fo:table-cell>
										<fo:table-cell xsl:use-attribute-sets="client.table.tf" text-align="right">
											<fo:block>
												<xsl:value-of select="grandAVB"/>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
								</fo:table-footer>
								<fo:table-body>
									<xsl:apply-templates select="shops/shop"/>
								</fo:table-body>
							</fo:table>
						</fo:block>
					</fo:block-container>
