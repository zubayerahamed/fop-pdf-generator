...
<!--!                                                              !-->
<!--!    File Name    : RM032.xsl                                  !-->
<!--!    Description  : BOM Listing Report                         !-->
...
<xsl:stylesheet 
	...
	<xsl:template match="boms/bom">
		<fo:table-row>
			<fo:table-cell xsl:use-attribute-sets="client.table.th" text-align="left">
				<fo:block><xsl:value-of select="bomId"/> : <xsl:value-of select="itemId"/> - <xsl:value-of select="itemName"/></fo:block>
			</fo:table-cell>
		</fo:table-row>
		<fo:table-row>
			<fo:table-cell>
